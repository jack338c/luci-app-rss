...
Package/luci-app-shadowsocksR-GFW = $(call Package/openwrt-ssr/Default,openssl,(OpenSSL),+libopenssl +libpthread +ipset +ip-full +iptables-mod-tproxy +libpcre +zlib +dnsmasq-full +coreutils +coreutils-base64 +curl +libmbedtls +libcurl +shadowsocksr-libev)
...
define Package/luci-app-shadowsocksR-GFW/install
	...
	$(INSTALL_DATA) ./files/root/etc/gfwlist/* $(1)/etc/gfwlist/
	$(INSTALL_DIR) $(1)/etc/init.d
	$(INSTALL_BIN) ./files/root/etc/init.d/* $(1)/etc/init.d/
	$(INSTALL_DIR) $(1)/etc/ipset
	$(INSTALL_DATA) ./files/root/etc/ipset/* $(1)/etc/ipset/
